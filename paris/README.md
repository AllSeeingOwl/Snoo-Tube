# How to Play Paris Pool & Use the Tracker App

## What is Paris Pool?
Paris Pool is a twist on traditional 8-ball pool, combining cue skill with geographic knowledge of Paris's iconic subway network. Players must call out a valid Paris Metro station that corresponds with the colour and type (solid/stripe) of the ball potted.

## How to Play

### 1. Basic Gameplay Integration
* Play standard 8-ball pool. Solids map to primary lines, stripes map to secondary lines.
* After potting a ball, call out a Paris Metro station matching the ball's color mapping.
* Stations can only be used once per game (unless modified by Tiering or Wildcard rules).
  * A valid station must be on the official Paris Metro map and match the colour rules.
...
* **Valid stations must:**
  * Be currently active on the official Paris Metro map.
...
* **RER & Tramways Wildcard Feature (Optional):**
  * Once per game, immediately after potting *any* coloured ball, a player may choose to call out a station on the **RER** or any **Tramway (T)** line.
...
The **Paris Pool Tracker** is a web-based companion app designed to help you keep track of stations as you play.
...
- **RER & Tramways Wildcard:** Easily unlock previously used stations using the built-in wildcard button.
...
4. Start the server by running:
   ...
   ```
   *(Or `node server.js`).*
5. Open your web browser and go to `http://localhost:3000/paris/`.
...
2. You may see a prompt at the bottom of the screen asking to "Add Paris Pool to Home screen". If so, tap it.
