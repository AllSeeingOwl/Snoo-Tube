...
const colourMap = {
    ...
    'blue': 'var(--blue-ball)',
    'purple': 'var(--purple-ball)',
    'orange': 'var(--orange-ball)',
...
async function fetchStations() {
    ...
    try {
        const response = await fetch('../data/stations.csv', { signal: controller.signal });
...
function parseCSV(str) {
    ...
    const boroughIdx = headers.indexOf('Borough');

    const stations = [];

    // Simple CSV parser handling basic commas (assuming no nested commas in this specific dataset's name/borough columns based on inspection,
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (name === undefined) continue; // Skip empty rows
        ...
        if (name === '') {
            ...
            stations.push({
                ...
                zone: parsedCols[boroughIdx] ? parsedCols[boroughIdx].replace(/^"|"$/g, '') : ''
            });
        }
    }
    ...
    for (let i = 0; i < stations.length; i++) {
        ...
        if (station.colours) {
            const validColours = ['red', 'yellow', 'green', 'brown', 'blue', 'purple', 'orange', 'black'];
            for (let j = 0; j < validColours.length; j++) {
                if (station.searchColours.includes(validColours[j])) {
                    station.parsedColours.push(validColours[j]);
                    if (typeof document !== 'undefined') {
                        const isSolid = station.searchColours.includes(validColours[j] + ' (solid)') || validColours[j] === 'black';
                        const isStripe = station.searchColours.includes(validColours[j] + ' (stripe)');

                        if (isSolid) {
                            const badge = createColourBadge(validColours[j], false);
                            if (badge) station.colourBadgesFragment.appendChild(badge);
                        }
                        if (isStripe) {
                            const badge = createColourBadge(validColours[j], true);
                            if (badge) station.colourBadgesFragment.appendChild(badge);
                        }
                    }
                }
...
function loadGameState() {
    ...
    let saved;
    try {
        saved = localStorage.getItem('tokyoPoolState');
    } catch (e) {
...
function saveGameState() {
    if (typeof localStorage === 'undefined') return;
    try {
        localStorage.setItem('tokyoPoolState', JSON.stringify(gameState));
    } catch (e) {
...
function createColourBadge(colourName, isStripe = false) {
    if (!colourMap[colourName]) return null;
    const badge = document.createElement('span');
    badge.className = 'colour-badge';
    if (isStripe) {
        badge.classList.add(`stripe-${colourName}`);
    } else {
        badge.style.backgroundColor = colourMap[colourName];
    }
    badge.title = isStripe ? `${colourName} (Stripe)` : `${colourName} (Solid)`;
...
function unlockStation(stationName) {
    if (confirm(`Use Yamanote Wildcard to unlock ${stationName}?`)) {
        // Reset count to 0
