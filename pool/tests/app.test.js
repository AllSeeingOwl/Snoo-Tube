...
test('parseCSV correctly parses a simple CSV string', (t) => {
    const csvData = `Station Name,Lines Served,Valid for Colours,Borough
...
test('parseCSV correctly handles quotes and commas within fields', (t) => {
    const csvData = `Station Name,Lines Served,Valid for Colours,Borough,"Times Used (This Game)"
...
test('parseCSV handles different column orders', (t) => {
    const csvData = `Borough,Valid for Colours,Station Name,Lines Served
...
test('parseCSV handles missing mandatory Station Name column', (t) => {
    const csvData = `Lines Served,Borough
Line 1,1
Line 2,2`;
...
Station A,Line 1,"Red, Red, purple, Black",1`;
    ...
    assert.deepStrictEqual(parsed[0].parsedColours, ['red', 'purple', 'black']);
});
...
test('saveGameState correctly saves gameState to localStorage', (t) => {
    ...
    try {
        app.saveGameState();
        assert.strictEqual(savedKey, 'poolSubwayState');
