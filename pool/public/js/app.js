...
async function fetchStations() {
    ...
    try {
        const response = await fetch('./data/Pool_Subway_Database.csv', { signal: controller.signal });
        ...
        if (station.colours) {
            ...
            for (let j = 0; j < validColours.length; j++) {
                if (station.searchColours.includes(validColours[j])) {
                    ...
                    if (typeof document !== 'undefined') {
                        // Check if it's a stripe or solid for the badge styling
                        const isStripe = station.searchColours.includes(validColours[j] + ' (stripe)');
                        const badge = createColourBadge(validColours[j], isStripe);
                        if (badge) {
...
function loadGameState() {
    ...
    let saved;
    try {
        saved = localStorage.getItem('poolSubwayState');
    } catch (e) {
...
function saveGameState() {
    if (typeof localStorage === 'undefined') return;
    try {
        localStorage.setItem('poolSubwayState', JSON.stringify(gameState));
    } catch (e) {
...
function unlockStation(stationName) {
    if (confirm(`Use Wildcard to unlock ${stationName}?`)) {
        // Reset count to 0
